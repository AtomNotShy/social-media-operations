...
export function TodayPage({ workspaceId }: { workspaceId: string }) {
  ...
  const introduction = <>
    <PageHeader eyebrow="工作区" title="今天必须处理的事" description={today.data ? `按 ${today.data.timezone} 时区汇总；这里不展示无行动意义的泛化数据。` : "自动发现与生产待办集中在这里。"} />
    <AutomationTodayPanel workspaceId={workspaceId} />
  </>;
  if (today.isLoading) return <>{introduction}<p>正在整理今天的工作…</p></>;
  if (today.error || !today.data) return <>{introduction}<ErrorState message="今日工作台加载失败。" onRetry={() => today.refetch()} /></>;
  return <>
    {introduction}
