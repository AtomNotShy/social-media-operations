...
export function useChannels(workspaceId: string) {
  return useQuery({
    ...
    queryFn: () =>
      workspaceId === "demo" ? clone(demoChannels) : service.listChannels(workspaceId),
  });
}
...
export function useChannel(workspaceId: string, channelId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      if (workspaceId !== "demo") return service.getChannel(workspaceId, channelId);
...
export function useCreateChannel(workspaceId: string) {
  ...
  return useMutation({
    mutationFn: async (input: OwnedChannelCreate) => {
      if (workspaceId !== "demo") return service.createChannel(workspaceId, input);
      const now = stamp();
      return {
        ...input,
...
export function useCreateTopic(workspaceId: string) {
  ...
  return useMutation({
    mutationFn: async (input: TopicCreate) => {
      if (workspaceId !== "demo") return service.createTopic(workspaceId, input);
      const now = stamp();
      return {
        ...input,
        id: crypto.randomUUID(),
...
export function useUpdateTopic(workspaceId: string, topicId: string) {
  ...
  return useMutation({
    mutationFn: async (input: TopicUpdate) => {
      if (workspaceId !== "demo") return service.updateTopic(workspaceId, topicId, input);
      const current =
        client.getQueryData<Topic>(queryKeys.production.topic(workspaceId, topicId)) ??
        clone(demoTopics).find((item) => item.id === topicId);
      if (!current) throw new Error("没有找到这个选题。");
      if (input.version !== current.version) throw new Error("选题已被其他成员修改，请刷新。");
      return {
        ...current,
        ...input,
    ...
    onSuccess: (updated) => {
      client.setQueryData(queryKeys.production.topic(workspaceId, topicId), updated);
      client.invalidateQueries({ queryKey: ["workspaces", workspaceId, "topics"] });
    },
  });
}
...
export function useProject(workspaceId: string, projectId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      if (workspaceId !== "demo") return service.getProject(workspaceId, projectId);
...
export function useCreateProject(workspaceId: string) {
  ...
  return useMutation({
    mutationFn: async (input: ContentProjectCreate) => {
      if (workspaceId !== "demo") return service.createProject(workspaceId, input);
      const now = stamp();
      return {
        ...input,
        id: crypto.randomUUID(),
...
export function useTransitionProject(workspaceId: string, projectId: string) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (workspaceId !== "demo") {
        return service.transitionProject(workspaceId, projectId, from, to, version);
      }
      const current =
      ...
      return { ...current, status: to, version: version + 1, updated_at: stamp() };
    },
    onSuccess: (updated) => {
      client.setQueryData(queryKeys.production.project(workspaceId, projectId), updated);
      client.invalidateQueries({
        queryKey: ["workspaces", workspaceId, "content-projects"],
      });
    },
...
export function useScripts(workspaceId: string, projectId: string) {
  return useQuery({
    ...
    queryFn: () =>
      workspaceId === "demo"
        ? clone(demoScripts).filter((item) => item.content_project_id === projectId)
...
export function useSaveScript(workspaceId: string, projectId: string) {
  ...
  return useMutation({
    mutationFn: async (input: ScriptCreate) => {
      if (workspaceId !== "demo") return service.createScript(workspaceId, projectId, input);
      const existing =
...
export function useUploadAsset(workspaceId: string, projectId?: string) {
  ...
  return useMutation({
        ...
        file.type.startsWith("image/")
          ...
          : file.type.startsWith("video/")
            ...
            : file.type.startsWith("audio/")
              ...
              : "document";
      if (workspaceId !== "demo") {
        return service.uploadAsset(workspaceId, file, projectId, type, rightsNote);
      }
      return {
        id: crypto.randomUUID(),
    ...
    onSuccess: () =>
      client.invalidateQueries({ queryKey: ["workspaces", workspaceId, "assets"] }),
  });
}
...
export function usePlans(workspaceId: string, status?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const items =
        workspaceId === "demo" ? clone(demoPlans) : await service.listPlans(workspaceId, status);
      return status ? items.filter((item) => item.status === status) : items;
    },
  });
}
  ...
  return {
    approve: useMutation({
      mutationFn: async (plan: PublishPlan) => {
        if (workspaceId !== "demo") return service.approvePlan(workspaceId, plan.id);
        return { ...plan, status: "approved", approved_at: stamp(), version: plan.version + 1 };
      },
      onSuccess: (updated) => {
        client.setQueriesData<PublishPlan[]>(
          { queryKey: ["workspaces", workspaceId, "publish-plans"] },
    ...
    package: useMutation({
      mutationFn: async (plan: PublishPlan) => {
        if (workspaceId !== "demo") return service.buildPackage(workspaceId, plan.id);
        const script =
          clone(demoScripts).find((item) => item.content_project_id === plan.content_project_id) ??
          clone(demoScripts)[0];
        return {
          plan_id: plan.id,
    ...
    mark: useMutation({
      mutationFn: async ({ plan, input }: { plan: PublishPlan; input: MarkPublished }) => {
        if (workspaceId !== "demo") return service.markPublished(workspaceId, plan.id, input);
        return {
          id: crypto.randomUUID(),
          ...
          result_payload: { matched_publish_package: input.matched_publish_package },
          created_by: null,
          created_at: stamp(),
        };
      },
      onSuccess: (record, variables) => {
        ...
        client.setQueryData(["workspaces", workspaceId, "publish-records", record.id], record);
      },
    }),
  };
}
...
export function useRecordReviews(workspaceId: string, recordId: string) {
  return useQuery({
    ...
    queryFn: () =>
      workspaceId === "demo"
        ? clone(demoReviews).filter((item) => item.publish_record_id === recordId)
...
export function usePublishRecord(workspaceId: string, recordId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      if (workspaceId !== "demo") return service.getRecord(workspaceId, recordId);
...
export function useCreateReview(workspaceId: string, recordId: string) {
  ...
  return useMutation({
    mutationFn: async (input: ReviewCreate) => {
      if (workspaceId !== "demo") return service.createReview(workspaceId, recordId, input);
      return {
        id: crypto.randomUUID(),
...
export function useCreateSavedView(workspaceId: string, entityType: string) {
  ...
  return useMutation({
    mutationFn: async (input: SavedViewCreate) => {
      if (workspaceId !== "demo") return service.createSavedView(workspaceId, input);
      const now = stamp();
      return {
        ...input,
        id: crypto.randomUUID(),
...
export function useUnifiedSearch(workspaceId: string, query: string) {
  return useQuery({
    ...
    queryFn: async () => {
      if (workspaceId !== "demo") return service.unifiedSearch(workspaceId, query);
      ...
      return [
        ...
        [item.title, item.snippet].some((text) => text?.toLowerCase().includes(needle)),
      );
    },
    staleTime: 15_000,
  });
...
export function useCreateExperiment(workspaceId: string) {
  ...
  return useMutation({
    mutationFn: async (input: ExperimentCreate) => {
      if (workspaceId !== "demo") return service.createExperiment(workspaceId, input);
      const now = stamp();
      return {
        ...input,
        id: crypto.randomUUID(),
