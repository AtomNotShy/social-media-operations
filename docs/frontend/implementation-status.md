...
| 阶段 | 已实现 |
|---|---|
| P0 | 仅限本地的开发身份、工作区创建与切换、Workbench Shell、标签栏、导航、对标账号、任务中心、权限可见性、统一错误处理 |
| P1 | 灵感库与详情、指标快照、评分、评论、逐字稿、L1/L2 分析、搜索与热榜、模式库、AI/ASR/Provider 用量 |
| P2 | 今日工作台、自有账号定位、选题、内容项目、脚本追加版本、内容包（生成/查看/编辑/冻结）、素材直传、审核排期、人工发布包、发布登记、复盘 |
| P3 | 跨实体关键词搜索、个人保存视图与共享视图读取、选题批量操作、键盘跳转、运营实验、项目负责人协作信息、移动端关键操作 |

工作区设置不是单独停留在 P0 的边界页。当前已接入：

- 工作区名称、时区、TikHub/AI 日预算读写。
- DeepSeek、OpenAI、OpenAI-Compatible 连接新增/编辑/启停/测试，以及 L1/L2/生成模型路由。
- Owner 成员列表、添加、角色调整和移除。
- TikHub 近 24 小时端点健康与任务队列健康。
- Owner 外部调用紧急暂停/恢复。
- 站内任务提醒边界与外部通知未连接状态。

设置页不再展示 P0/P1/P2/P3 开发阶段标签。邮件、Slack 等独立通知因没有后端配置接口，仍明确标记为“未连接”。

### 2.1 P2 真实性约束

- AI 脚本生成返回异步任务；前端显示 Job 已进入队列，不显示“脚本已完成”。
- 脚本工作台提供“脚本编辑 / 内容包”两个页签；内容包从定稿脚本按目标平台
  生成，展示分镜、标题候选、封面、话题、正文、素材需求与配音/BGM 提示。
- 内容包编辑保存会追加新版本并自动选中；冻结后版本只读。渲染适配器
  （Replicast/HyperFrames）与素材检索仍未在前端接入。
- 脚本生成会解析选题的来源证据（`content:`/`inspiration:` 引用）为
  `source_materials` 上下文包，模型基于原文正文改写；证据引用无法解析时
  返回 409 `EVIDENCE_UNAVAILABLE`，不会在缺失材料时静默生成。
- 复盘生成会把被复盘项目的发布脚本与发布包放进上下文包（review-v2），
  只基于提供的指标和正文复盘，不编造趋势或原因。
- 全部 AI 提示词由统一 Prompt Registry 管理并版本化；每次 AI 调用写入
  `ai_attempt_logs` 分次日志（token/延迟/成本/错误码），预算预留按上下文
  大小与模型单价估算，而非固定任务价格。
- 人工脚本保存追加新版本，不覆盖历史版本。
...
`/login` 只在 `NEXT_PUBLIC_APP_ENV=local` 的本地构建中展示调试身份表单；未显式标记为 `local` 的构建访问该路径会重定向至 `/w/demo/today`。生产工作台不显示“连接开发后端”“连接真实后端”或“连接本地后端”等入口，且不会默认向访问者本机的 `127.0.0.1` 发起 API 请求。
...
站点默认进入 `demo` 工作区；演示数据只用于展示交互契约。真实工作区必须连接可访问的后端 API，并由后端提供真实实体、任务、对象存储上传意图和发布记录。
