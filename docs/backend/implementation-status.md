...
- Ruff 通过。
- 181 项后端测试通过；2 项 PostgreSQL 集成测试和 1 项 PostgreSQL 性能测试在普通套件中跳过，并在 PostgreSQL 环境单独通过。
- SQLite 空库可从 0001 顺序升级至 0022。
- PostgreSQL 16 容器可在线升级至 0022（本次新增迁移为纯加列，已通过 SQLite 顺序升级与 `alembic check` 验证；PostgreSQL 在线升级沿用既有容器证据）。
- SQLite 与 PostgreSQL 的 `alembic check` 均无模型/迁移漂移。
- OpenAPI 可生成 112 条路径、145 个操作；文档中的 113 个操作均有自动契约覆盖，前端 TypeScript 类型检查通过。
